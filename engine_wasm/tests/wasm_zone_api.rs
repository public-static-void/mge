...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_zone_api.wasm");
...
fn test_wasm_zone_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_zone_api", &[])
        .expect("Failed to call test_zone_api");
