...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_craft_api.wasm");
...
fn test_wasm_craft_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_craft_api", &[])
        .expect("Failed to call test_craft_api");
