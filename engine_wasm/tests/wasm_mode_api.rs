...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_mode_api.wasm");
...
fn test_wasm_mode_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_mode_api", &[])
        .expect("Failed to call test_mode_api");
