...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_death_decay_api.wasm");
...
#[test]
fn test_wasm_death_decay_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_death_decay_api", &[])
        .expect("Failed to call test_death_decay_api");
