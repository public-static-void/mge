...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_job_cancel.wasm");
...
#[test]
fn test_wasm_job_cancel_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_job_cancel", &[])
        .expect("Failed to call test_job_cancel");
