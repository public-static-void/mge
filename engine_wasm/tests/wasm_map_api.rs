...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_map_api.wasm");
...
fn test_wasm_map_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_map_api", &[])
        .expect("Failed to call test_map_api");
