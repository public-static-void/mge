...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_body_part_damage.wasm");
...
#[test]
fn test_wasm_body_part_damage_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_body_part_damage", &[])
        .expect("Failed to call test_body_part_damage");
