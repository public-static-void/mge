...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_economic_api.wasm");
...
#[test]
fn test_wasm_economic_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_economic_api", &[])
        .expect("Failed to call test_economic_api");
