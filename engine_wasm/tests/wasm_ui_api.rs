...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_ui.wasm");
...
fn test_wasm_ui_api() {
    ...
    // Call exported UI test function in WASM which exercises ui, ui_tree, and ui_events imports
    let result = engine
        .invoke_exported_function("test_ui_api", &[])
        .expect("Failed to call test_ui_api");
