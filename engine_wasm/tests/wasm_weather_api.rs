...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_weather_api.wasm");
...
#[test]
fn test_wasm_weather_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_weather_api", &[])
        .expect("Failed to call test_weather_api");
