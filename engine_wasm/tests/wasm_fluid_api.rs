...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_fluid_api.wasm");
...
fn test_wasm_fluid_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_fluid_api", &[])
        .expect("Failed to call test_fluid_api");
