...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_faction_api.wasm");
...
#[test]
fn test_wasm_faction_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_faction_api", &[])
        .expect("Failed to call test_faction_api");
