...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_multiscale_map.wasm");
...
#[test]
fn test_wasm_multiscale_map_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_multiscale_map", &[])
        .expect("Failed to call test_multiscale_map");
