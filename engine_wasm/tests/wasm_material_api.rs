...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_material_api.wasm");
...
#[test]
fn test_wasm_material_api_bridge() {
    let wasm_file = compile_test_wasm();

    // Materials are auto-loaded from the "materials" sibling of schema_path
    ...
    let result = engine
        .invoke_exported_function("test_material_api", &[])
        .expect("Failed to call test_material_api");
