...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_input_api.wasm");
...
fn test_wasm_input_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_input_api", &[])
        .expect("Failed to call test_input_api");
