...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_inventory_api.wasm");
...
#[test]
fn test_wasm_inventory_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_inventory_api", &[])
        .expect("Failed to call test_inventory_api");
