...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_fog_api.wasm");
...
fn test_wasm_fog_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_fog_api", &[])
        .expect("Failed to call test_fog_api");
