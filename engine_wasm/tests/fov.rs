...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_fov_api.wasm");
...
fn test_wasm_fov_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_fov_api", &[])
        .expect("Failed to call test_fov_api");
