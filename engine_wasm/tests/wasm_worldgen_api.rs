...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_worldgen_api.wasm");
...
#[test]
fn test_wasm_worldgen_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_worldgen_api", &[])
        .expect("Failed to call test_worldgen_api");
