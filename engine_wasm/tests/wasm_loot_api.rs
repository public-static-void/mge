...
fn compile_test_wasm() -> NamedTempFile {
    let wasm_bytes = load_wasm_test_artifact("test_loot_api.wasm");
...
fn test_wasm_loot_api_bridge() {
    ...
    let result = engine
        .invoke_exported_function("test_loot_api", &[])
        .expect("Failed to call test_loot_api");
