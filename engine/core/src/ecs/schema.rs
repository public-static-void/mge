...
/// Validates that the JSON is a schema with a title, no invalid property constraints,
/// and that all modes are in the provided allowed_modes list.
fn validate_schema(schema_json: &str, allowed_modes: &[String]) -> Result<(), String> {
    let value: Value =
...
pub fn load_schemas_recursively<P: AsRef<Path>>(
    ...
    for entry in walkdir::WalkDir::new(dir) {
        ...
        if path.is_file() && path.extension().and_then(|e| e.to_str()) == Some("json") {
            ...
            if validate {
                validate_schema(&data, allowed_modes)
