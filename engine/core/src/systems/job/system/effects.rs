...
/// Recursively processes an effect and all its chained effects.
fn process_effect_and_chains(
    effect_registry: &Arc<
        Mutex<crate::systems::job::registry::effect_processor_registry::EffectProcessorRegistry>,
    >,
    world: &mut World,
    job_id: u32,
    effect: &JsonValue,
    ...
    effect_registry
        .lock()
        .unwrap()
        .process_effects(world, job_id, std::slice::from_ref(effect));
...
/// Applies all effects (and their chains) for a job.
fn apply_all_effects(
    ...
    let mut applied_effect_indices = Vec::new();
...
pub fn process_job_effects(
    ...
    // If the job is in a terminal state, apply all effects at once.
    let state = job.get("state").and_then(|v| v.as_str()).unwrap_or("");
    if matches!(state, "complete" | "failed" | "cancelled" | "interrupted") {
        apply_all_effects(world, job_id, job_type, job);
        return;
    }

    // --- Incremental: Apply only one effect per tick for non-terminal jobs ---
