...
pub fn handle_fetching_resources_state(
    ...
    // If this job does not require resources or has no reservation, skip fetching phase.
    if requirements.is_empty() || reserved_stockpile.is_none() || assigned_to == 0 {
        job["state"] = json!("in_progress");
        return job;
    }

    let reserved_stockpile = reserved_stockpile.unwrap();

    let agent_pos = world.get_component(assigned_to, "Position");
    let stockpile_pos = world.get_component(reserved_stockpile, "Position");
