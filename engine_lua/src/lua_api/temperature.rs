...
pub fn register_temperature_api(
    ...
    // get_cell_temperature(x, y, z?) -> f64 (per-cell temperature from the
    // transient diffusion map, or global ambient when the map is empty or the
    // cell is absent; z defaults to 0 when omitted)
    let w = world;
    let get_cell_temperature =
        lua.create_function_mut(move |_, (x, y, z): (i32, i32, Option<i32>)| {
            let world = w.borrow();
            Ok(world.get_cell_temperature(x, y, z.unwrap_or(0)))
        })?;
    globals.set("get_cell_temperature", get_cell_temperature)?;
