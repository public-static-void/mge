...
pub fn register_loot_api(lua: &Lua, globals: &Table, world: Rc<RefCell<World>>) -> LuaResult<()> {
        ...
        move |_, (name, entries): (String, Table)| -> LuaResult<bool> {
            ...
            for pair in entries.pairs::<Value, Value>() {
                ...
                let entry_table = match val {
                    Value::Table(t) => t,
                    _ => return Err(mlua::Error::external("each entry must be a table")),
                };

                let item_id: String = entry_table
                    .get("item_id")
                    .map_err(|e| mlua::Error::external(format!("entry missing 'item_id': {e}")))?;
                let weight: u32 = entry_table
                    .get("weight")
                    .map_err(|e| mlua::Error::external(format!("entry missing 'weight': {e}")))?;
    ...
    let roll_fn = lua.create_function_mut(move |lua: &Lua, name: String| -> LuaResult<Table> {
