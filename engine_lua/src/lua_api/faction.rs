...
pub fn register_faction_api(
    ...
    let w = world.clone();
    let modify_reputation_fn = lua.create_function_mut(
        move |_, (entity, faction_id, delta): (u32, String, i64)| {
            let mut world = w.borrow_mut();
    ...
    let w = world;
    let get_reputation_fn = lua.create_function_mut(
        move |_, (entity, faction_id): (u32, String)| {
            let world = w.borrow();
