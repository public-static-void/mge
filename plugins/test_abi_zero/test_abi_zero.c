// test_abi_zero.c — TEST ONLY plugin with abi_version = 0
//
...
//
// This plugin sets abi_version to 0 to verify that the engine rejects
// plugins that were compiled without a proper version field (pre-versioning
// detection) or that intentionally set the version to 0.
...
static int init(struct EngineApi *api, void *world) {
  (void)api;
  (void)world;
  printf("[TEST ABI ZERO] init called (should never happen)\n");
  return 0;
}
...
// Runtime initialization — uses abi_version = 0
__attribute__((constructor)) void init_vtable() {
  vtable.abi_version = 0; // Zero — simulates pre-versioning plugin
  vtable.init = init;
